	...
	function renderBlogEntry($blog, $homepage, $preview=false) {
	    ...
	    if ($blog_image) {
	    	...
	    	if ($preview)
	  		...
	  		$image_html .= "<img src='{$thumb->url}' imagegalleryurl='{$blog_image->url}' alt='{$thumb->description}' style='width:100%; cursor:zoom-in' onclick='showimagegallerymodal(this)'>";
	    }
        ...
        if (count($blog->images)>1) {
        	...
        	foreach ($blog->images as $image) {
        		...
        		$blog_images .= "<img src='{$thumb->url}' imagegalleryurl='{$image->url}' class='w3-image w3-padding' onclick='showimagegallerymodal(this)' style='cursor:zoom-in'>";
        	}
        	$blog_images .= '</div>';
        }
